...
public class ListCommand extends Program implements ListExecutable {
    ...
    public ListCommand(StorageApiConsole console, String src, LIST_MODE mode) {
        super();
        this.mConsole = (StorageApiConsole) console;
        ...
        if (this.mMode.compareTo(LIST_MODE.DIRECTORY) == 0) {
            ...
            if (documents != null && !documents.isEmpty()) {
                for (Document document : documents) {
                    FileSystemObject fso = FileHelper.createFileSystemObject(document);
                    if (fso != null) {
                        if (isTrace()) {
            ...
            // TODO: Add parent directory reference to list
            /*if (this.mSrc != null &&
                    this.mSrc.compareTo(FileHelper.ROOT_DIRECTORY) != 0 &&
                    this.mMode.compareTo(LIST_MODE.DIRECTORY) == 0) {
                this.mFiles.add(0, new ParentDirectory(current.getParent()));
            }*/

        } else {
            // Build the parent information
            FileSystemObject fso = FileHelper.createFileSystemObject(current);
            if (fso != null) {
                if (isTrace()) {
                    Log.v(TAG, String.valueOf(fso));
                }
