...
import com.cyanogenmod.filemanager.*;
import com.cyanogenmod.filemanager.adapters.*;
import com.cyanogenmod.filemanager.console.*;
import com.cyanogenmod.filemanager.console.secure.*;
...
import com.cyanogenmod.filemanager.ui.dialogs.*;
import com.cyanogenmod.filemanager.ui.policy.*;
...
public class NavFrag extends android.support.v4.app.Fragment
        ...
        NavigationView.OnNavigationRequestMenuListener, NavigationView.OnNavigationSelectionChangedListener {
        ...
        public void onReceive(android.content.Context context, Intent intent) {
            if (intent != null) {
                if (intent.getAction().compareTo(
                    ...
                    try {
                        FileSystemObject fso = com.cyanogenmod.filemanager.util.CommandHelper.getFileInfo(context, file, null);
                        if (fso != null) {
     ...
     * @hide
     */
    static java.util.Map<MimeTypeHelper.MimeTypeCategory, android.graphics.drawable.Drawable> EASY_MODE_ICONS = new
            java.util.HashMap<MimeTypeHelper.MimeTypeCategory, android.graphics.drawable.Drawable>();
    ...
    private static final List<MimeTypeHelper.MimeTypeCategory> EASY_MODE_LIST = new ArrayList<MimeTypeHelper.MimeTypeCategory>() {
        {
            add(NONE);
            add(IMAGE);
            add(VIDEO);
    ...
    public View onCreateView(
            android.view.LayoutInflater inflater, ViewGroup container,
            Bundle savedInstanceState) {

        mLayoutInflater = inflater;

        if (DEBUG) {
            Log.d(TAG, "NavFrag.onCreate"); //$NON-NLS-1$
        }
        ...
        ThemeManager.Theme theme = com.cyanogenmod.filemanager.ui
            ...
            mNfcAdapter.setBeamPushUrisCallback(new NfcAdapter.CreateBeamUrisCallback() {
                ...
                public android.net.Uri[] createBeamUris(android.nfc.NfcEvent event) {
                    ...
                    if (selectedFiles.size() > 0) {
                        List<android.net.Uri> fileUri = new ArrayList<android.net.Uri>();
        ...
        mToolBar = ((NavigationActivity) getActivity())
                .mToolbar;
    ...
    public void onDestroy() {
        if (DEBUG) {
            Log.d(TAG, "NavFrag.onDestroy"); //$NON-NLS-1$
        }
    ...
    private void addBookmarkToDrawer(Bookmark bookmark) {
        ...
        view.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    FileSystemObject fso = com.cyanogenmod.filemanager.util.CommandHelper.getFileInfo(
    ...
    private List<Bookmark> loadUserBookmarks() {
        List<Bookmark> bookmarks = new ArrayList<Bookmark>();
        android.database.Cursor cursor = Bookmarks.getAllBookmarks(getActivity().getContentResolver());
        try {
        ...
        try {
            ...
            if (!com.cyanogenmod.filemanager.activities.NavFrag.this.mChRooted) {
                ...
                DialogHelper.showToast(
                        getActivity(),
                        R.string.msgs_cant_create_console, android.widget.Toast.LENGTH_LONG);
                exit();
                return;
            }
    ...
    void applyInitialDir(final NavigationView navigationView, final Intent intent) {
        ...
        com.cyanogenmod.filemanager.console.VirtualConsole vc = VirtualMountPointConsole.getVirtualConsoleForPath(initialDir);
        ...
        dialog.setOnMountListener(new FilesystemInfoDialog.OnMountListener() {
            ...
            public void onRemount(MountPoint mountPoint) {
                ...
                if (mountPoint.isSecure()) {
                    ...
                    Intent intent = new Intent();
                    intent.putExtra(EXTRA_ADD_TO_HISTORY, false);
                    initNavigation(NavFrag.this.mCurrentNavigationView, false, intent);
                }
            }
        });
        dialog.show();
    ...
    private boolean checkBackAction() {
        ...
        if (this.mExitFlag && (this.mExitFlag != flag || timeout)) {
            ...
            DialogHelper.showToast(getActivity(), R.string.msgs_push_again_to_exit, android.widget.Toast.LENGTH_SHORT);
            if (mNeedsEasyMode) {
    ...
    public synchronized boolean navigateToHistory(History history) {
        try {
            ...
            if (realHistory.getItem() instanceof NavigationViewInfoParcelable) {
                ...
                searchIntent.putExtra(SearchActivity.EXTRA_SEARCH_RESTORE, (android.os.Parcelable)info);
                startActivityForResult(searchIntent, INTENT_REQUEST_SEARCH);
            } else {
                //The type is unknown
            ...
            this.mHandler.post(new Runnable() {
                ...
                public void run() {
                    DialogHelper.showToast(
                            getActivity(),
                            R.string.msgs_history_unknown, android.widget.Toast.LENGTH_LONG);
                }
            });
    ...
    public boolean back() {
        ...
        while (this.mHistory.size() > 0) {
            ...
            if (h.getItem() instanceof NavigationViewInfoParcelable) {
                ...
                try {
                    FileSystemObject info = com.cyanogenmod
    ...
    public void openActionsDialog(Object item, boolean global) {
        ...
        com.cyanogenmod.filemanager.ui.dialogs.ActionsDialog dialog = new com.cyanogenmod
                .filemanager.ui.dialogs.ActionsDialog(getActivity(), this,
                fso,
                global, false);
    ...
    private void onLayoutChanged() {
        ...
        if (this.mOrientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE) {
            // Landscape mode
            ViewGroup statusBar = (ViewGroup)mView.findViewById(
                    R.id.navigation_statusbar);
    ...
    void applyTheme() {
        ...
        //- Layout
        View v = mView.findViewById(R.id.navigation_layout);
        ...
        //- StatusBar
        v = mView.findViewById(R.id.navigation_statusbar);
        if (orientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE) {
        ...
        v = mView.findViewById(R.id.ab_overflow);
        ...
        theme.setImageDrawable(getActivity(), (ImageView)v, "expander_close_drawable"); //$NON-NLS-1$
        ...
        theme.setImageDrawable(getActivity(), (ImageView)v, "ab_layout_mode_drawable"); //$NON-NLS-1$
        v = titleLayout.findViewById(R.id.ab_view_options);
        theme.setImageDrawable(getActivity(), (ImageView)v, "ab_view_options_drawable"); //$NON-NLS-1$
        ...
        theme.setImageDrawable(getActivity(), (ImageView)v, "ab_selection_done_drawable"); //$NON-NLS-1$
        ...
        // - Navigation drawer
        //v = mView.findViewById(R.id.history_empty);
        //theme.setTextColor(getActivity(), (TextView)v, "text_color"); //$NON-NLS-1$
    ...
    private void applyTabTheme() {
        ...
        /*View v = mView.findViewById(R.id.drawer);
        theme.setBackgroundDrawable(getActivity(), v, "background_drawable"); //$NON-NLS-1$

        v = mView.findViewById(R.id.drawer_bookmarks_tab);
        theme.setTextColor(getActivity(), (TextView)v, "text_color"); //$NON-NLS-1$
        v = mView.findViewById(R.id.drawer_history_tab);
        theme.setTextColor(getActivity(), (TextView)v, "text_color"); //$NON-NLS-1$

        v = mView.findViewById(R.id.ab_settings);
        theme.setImageDrawable(getActivity(), (com.cyanogenmod.filemanager.ui.widgets.ButtonItem) v, "ab_settings_drawable"); //$NON-NLS-1$
        v = mView.findViewById(R.id.ab_clear_history);
